    ...
    getQuoteDetails({ recordId: this.quoteId })
        .then(result => {
            ...
            if (result) {
                ...
                if (result.QuoteForce__Primary_Image__c) {
                    this.primaryImageUrl = result.QuoteForce__Primary_Image__c;
                }

                if (result.QuoteForce__Secondary_Image__c) {
    ...
    handleClickPages(event) {
        ...
        if (this.quoteDetailPageSelected == true) {
            ...
            if (childCmp) {
                ...
                console.log("handle click pages"+this.quoteId);

        if (JSON.stringify(dataLabel).includes("Custom Page")) {
            getCustomPage({ recordId: this.quoteid })
                .then(result => {
                    result.forEach(element => {
                        console.log('element id'+element.Id+' check current custom page'+ this.currentCustomPageRecordId);
                        if (element.Id == this.currentCustomPageRecordId) {
